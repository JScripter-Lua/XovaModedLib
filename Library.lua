...
function Library.Effect(c, p)
    -- Freeze AutomaticSize before spawning the ripple.
    -- The growing circle child would otherwise stretch any container
    -- that has AutomaticSize.X (e.g. the Button pill frame).
    local wasAuto  = p.AutomaticSize
    local frozenSz = p.AbsoluteSize
    if wasAuto ~= Enum.AutomaticSize.None then
        p.AutomaticSize = Enum.AutomaticSize.None
        p.Size = UDim2.new(0, frozenSz.X, 0, frozenSz.Y)
    end
    ...
    if rx < 0 or ry < 0 or rx > c.AbsoluteSize.X or ry > c.AbsoluteSize.Y then
        if wasAuto ~= Enum.AutomaticSize.None then
            p.AutomaticSize = wasAuto
            p.Size = UDim2.new(0, 0, 0, frozenSz.Y)
        end
        return
    end
    ...
    local t = TweenService:Create(circle, TweenInfo.new(2.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {
        Size                   = UDim2.new(0, frozenSz.X * 1.5, 0, frozenSz.X * 1.5),
        BackgroundTransparency = 1
    })
...
--]]
--[[
    GetIcon(name) — Universal icon resolver. No "rbxassetid://" prefix needed.
    Just pass the icon name:

        GetIcon("eye")               -> "rbxassetid://10723346959"  (lucide-eye)
        GetIcon("lucide-home")       -> "rbxassetid://10723407389"  (exact key)
        GetIcon("rbxassetid://999")  -> "rbxassetid://999"          (passthrough)
        GetIcon(nil) / GetIcon("")   -> ""                          (safe empty)

    Lookup order:
        1. LucideIcons["lucide-" .. name]   "eye"         -> lucide-eye asset
        2. LucideIcons[name]                "lucide-home" -> exact key
        3. tostring(name)                   passthrough for raw rbxassetid strings

    IMPORTANT: returns "" for nil/empty — never nil.
    "" is truthy in Lua, so guard with  result ~= ""  before using as Image.
--]]
...
--[[
    ...
    Args:
        ...
        BbIcon            (string)         - Icon on the toggle pill button
                                             Accepts: "eye"  /  "lucide-home"  /  "rbxassetid://..."
                                             Default: Vita logo
...
function Library:Window(Args)
    ...
    -- BbIcon: icon shown on the toggle pill button.
    -- Accepts a Lucide icon name ("eye"), "lucide-home", or a full "rbxassetid://..." string.
    -- Falls back to the default Vita logo when not provided.
    local BbIconRaw    = Args.BbIcon
    local DefaultPillIcon = "rbxassetid://104055321996495"
    ...
    -- PillLogo: customizable via Args.BbIcon — accepts Lucide name, exact key, or rbxassetid string
    local PillLogo = Library:Create("ImageLabel", {
        ...
        ImageColor3            = T.Accent,
        Image                  = (BbIconRaw and GetIcon(BbIconRaw) ~= "" and GetIcon(BbIconRaw)) or DefaultPillIcon
    })
    trackAccent(PillLogo, "ImageColor3")
    ...
    --[[
        Window:NewPage(Args)
          Title    (string)            Tab card title
          Desc     (string)            Tab card subtitle
          Icon     (string|number)     Lucide name — no rbxassetid prefix needed
                                       e.g.  "eye", "lucide-home", "rbxassetid://123"
          TabImage (Color3|"#RRGGBB")  Per-tab banner tint — overrides global T.TabImage
                                       e.g.  TabImage = "#3282FF"
    --]]
    function Window:NewPage(Args)
        ...
        -- Per-tab TabImage: overrides global T.TabImage for this tab only
        local PageTabImage = Args.TabImage and ResolveColor(Args.TabImage) or nil
        ...
        local TabBanner = Library:Create("ImageLabel", {
            ...
            -- Per-page override takes priority; falls back to global T.TabImage
            ImageColor3            = PageTabImage or T.TabImage,
            ScaleType              = Enum.ScaleType.Crop
        })
        -- Only track globally when no per-page color is set
        if not PageTabImage then
        ...
        function Page:Button(Args)
            ...
            -- Button pill icon — only Args.ButtonIcon places an icon inside the pill.
            -- Args.Icon is already used as the left row icon (RowIcon above).
            -- GetIcon(nil) returns "" which is truthy in Lua, so we must check ~= "".
            local btnIconId = Args.ButtonIcon and GetIcon(Args.ButtonIcon) or ""
            if btnIconId ~= "" then
            ...
            local BtnLbl = Library:Create("TextLabel", {
                ...
                BorderSizePixel        = 0,
                LayoutOrder            = 0,
                -- AutomaticSize.X only: grows horizontally with text.
                -- XY caused a circular dependency with Btn.AutomaticSize.X,
                -- letting text grow taller than the pill and overflow outside it.
            ...
            -- AutomaticSize.X lets the pill grow to fit icon + text + padding.
            -- Fixed height (25px) is never affected by AutomaticSize.
    ...
    --[[
        ...
        Changes the icon displayed on the toggle pill button at runtime.
        Accepts the same formats as Args.BbIcon:
            Library:SetPillIcon("eye")                    -- Lucide shorthand
            Library:SetPillIcon("lucide-home")            -- exact Lucide key
            Library:SetPillIcon("rbxassetid://123456789") -- raw asset string
            Library:SetPillIcon(nil)                      -- restore default Vita logo
    --]]
    function Library:SetPillIcon(name)
        if not name then
            PillLogo.Image = DefaultPillIcon
        else
            local resolved = GetIcon(name)
            PillLogo.Image = (resolved and resolved ~= "") and resolved or DefaultPillIcon
        end
    end
